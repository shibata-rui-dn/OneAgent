/**
 * APIルート定義（完全版 + ツール直接実行機能追加 + 会話履歴対応）
...
function setupAgentRoutes(app, aiAgent, toolManager, oauthSystem) {
  ...
  // AIエージェント（認証必須、.envフォールバック対応、会話履歴対応）
  app.post('/agent',
    oauthSystem.middleware.auth,
    async (req, res) => {
      try {
        ...
        // ✅ 会話履歴対応：messagesパラメータを追加
        const { query, streaming, model, temperature, maxTokens, tools, messages } = req.body;
        ...
        const options = {
          ...
          // ✅ 会話履歴を追加
          conversationHistory: messages || [] // クライアントから送信された会話履歴
        };
        ...
        console.log(`📝 処理オプション: streaming=${options.streaming}, model=${options.model}, fallback=${authContext.fallbackUsed}, conversationHistory=${options.conversationHistory.length}件`);
  ...
  console.log('🤖 AIエージェントエンドポイント（.envフォールバック対応強化版 + 会話履歴対応）を設定しました');
}
      ...
      const systemInfo = {
        ...
        endpoints: {
          ...
          api: {
            ...
            directToolExecution: '/tools/execute/:toolName',
            batchToolExecution: '/tools/execute/batch'
          },
          mcp: '/mcp'
        }
      };
