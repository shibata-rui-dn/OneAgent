 ...
 * 動的ツール読み込み、実行、管理機能
 * install-secure-file-manager.js自動実行機能
 */
...
export class ToolManager {
  ...
  async loadTools() {
    try {
      ...
      const hasSecureFileManager = toolDirs.some(dir => dir.name === 'secure_user_file_manager');
      if (!hasSecureFileManager) {
        console.log('🔍 secure_user_file_manager が見つかりません。自動インストールします...');
        await this.runSecureFileManagerInstaller();
  ...
  async runSecureFileManagerInstaller() {
    return new Promise((resolve, reject) => {
      console.log('🛠️ install-secure-file-manager.js を自動実行中...');
      
      // プロジェクトルートディレクトリを取得
      const projectRoot = path.resolve(__dirname, '../../');
      const installerScript = path.join(projectRoot, 'install-secure-file-manager.js');
      ...
      // child_processでnode install-secure-file-manager.jsを実行
  ...
  displayToolInstallationGuide() {
    ...
    console.log('   node install-secure-file-manager.js');
    ...
    console.log('   • secure_user_file_manager (OAuth認証セキュアファイル管理)');
  ...
  async loadTool(toolName, toolPath) {
    try {
      ...
      const toolInfo = {
        ...
        autoInstalled: toolName === 'secure_user_file_manager' && this.stats.autoInstalled > 0
      };
  ...
  getStatistics() {
    ...
    return {
      ...
      autoInstall: {
        ...
        secureFileManagerInstalled: this.tools.has('secure_user_file_manager'),
        installationCount: this.stats.autoInstalled,
        scriptPath: 'install-secure-file-manager.js' // 🔧 追加: スクリプトパス
      },
      installationGuide: {
        autoCommand: 'node install-secure-file-manager.js',
        manualCommand: 'node install-secure-file-manager.js',
